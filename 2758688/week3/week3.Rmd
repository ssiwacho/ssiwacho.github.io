...
.right-column[
```{r message=F, warning=F, fig.retina=3, fig.width=14, fig.height=8}
library(GGally)
ggpairs(dat2%>%select(-longitude,-latitude)%>%select(1:6,8,7),switch="y")
```
...
```{r fig.retina=3, fit,width=12, fig.height=6}
ggplot(dat2%>%filter(median_house_value<500000),
